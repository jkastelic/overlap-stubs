...
void Histos::fillTrackCands(const InputData& inputData, const matrix<Sector>& mSectors, const matrix<HTpair>& mHtPairs) {
  ...
  for (unsigned int iEtaReg = 0; iEtaReg < settings_->numEtaRegions(); iEtaReg++) {
    ...
    for (unsigned int iPhiSec = 0; iPhiSec < settings_->numPhiSectors(); iPhiSec++) {
      const HTpair& htPair = mHtPairs(iPhiSec, iEtaReg);
      // Get number of tracks in this sector and add it to running sum.
  ...
  for (unsigned int iEtaReg = 0; iEtaReg < settings_->numEtaRegions(); iEtaReg++) {
    ...
    for (unsigned int iPhiSec = 0; iPhiSec < settings_->numPhiSectors(); iPhiSec++) {
      const HTpair& htPair = mHtPairs(iPhiSec, iEtaReg);
      hisStubsOnTracksPerSect_->Fill(htPair.numStubsOnTrackCands3D()); // Number of stubs assigned to tracks in this sector.
      nStubsOnTracksInEtaReg += htPair.numStubsOnTrackCands3D();
    }
...
//  for ( std::vector<std::string>::const_iterator lHitIt = fitList_.begin(); lHitIt != fitList_.end(); ++lHitIt ) {
  for(auto &fitName : settings_->trackFitters() ){
    ...
    hisFitVsTrueQinvPtGenCand_[fitName] = inputDir.make<TH2F>(("FitVsTrueQinvPtGenCand_"+(fitName)).c_str(), "; TP q/p_{T} (Genuine Cand); Fitted q/p_{T}", 100, -0.5, 0.5, 100, -0.5, 0.5 );
    hisFitVsTruePhi0GenCand_[fitName]   = inputDir.make<TH2F>(("FitVsTruePhi0GenCand_"+(fitName)).c_str(), "; TP #phi_{0} (Genuine Cand); Fitted #phi_{0}", 70, -3.5, 3.5, 70, -3.5, 3.5 );
    hisFitVsTrueD0GenCand_[fitName]     = inputDir.make<TH2F>(("FitVsTrueD0GenCand_"+(fitName)).c_str(), "; TP d_{0} (Genuine Cand); Fitted d_{0}", 100, -1., 1., 100, -1., 1. );
    hisFitVsTrueZ0GenCand_[fitName]     = inputDir.make<TH2F>(("FitVsTrueZ0GenCand_"+(fitName)).c_str(), "; TP z_{0} (Genuine Cand); Fitted z_{0}", 100, -25., 25., 100, -25., 25. );
    hisFitVsTrueEtaGenCand_[fitName]    = inputDir.make<TH2F>(("FitVsTrueEtaGenCand_"+(fitName)).c_str(), "; TP #eta (Genuine Cand); Fitted #eta", 70, -3.5, 3.5, 70, -3.5, 3.5 ); 
    ...
    hisTrueVsSeedQinvPtResGoodChi2_[fitName] = inputDir.make<TH1F>(("TrueVsSeedQinvPtResGoodChi2_"+(fitName)).c_str(), "True minus seed q/p_{T} (good #chi^2)", 100, -0.5, 0.5 );
    hisTrueVsSeedPhi0ResGoodChi2_[fitName]   = inputDir.make<TH1F>(("TrueVsSeedPhi0ResGoodChi2_"+(fitName)).c_str(), "True minus seed #phi_{0} (good #chi^2)", 100, -0.1, 0.1 );
    hisTrueVsSeedD0ResGoodChi2_[fitName]     = inputDir.make<TH1F>(("TrueVsSeedD0ResGoodChi2_"+(fitName)).c_str(), "True minus seed d_{0} (good #chi^2)", 100,  -1., 1. );
    hisTrueVsSeedZ0ResGoodChi2_[fitName]     = inputDir.make<TH1F>(("TrueVsSeedZ0ResGoodChi2_"+(fitName)).c_str(), "True minus seed z_{0} (good #chi^2)", 100, -10., 10. );
    hisTrueVsSeedEtaResGoodChi2_[fitName]    = inputDir.make<TH1F>(("TrueVsSeedEtaResGoodChi2_"+(fitName)).c_str(), "True minus seed #eta (good #chi^2)", 100, -0.1, 0.1 );
 
    hisFitVsTrueQinvPtFakeCand_[fitName] = inputDir.make<TH2F>(("FitVsTrueQinvPtFakeCand_"+(fitName)).c_str(), "; TP q/p_{T} (Fake Cand); Fitted q/p_{T}", 100, -0.5, 0.5, 100, -0.5, 0.5 );
    hisFitVsTruePhi0FakeCand_[fitName]   = inputDir.make<TH2F>(("FitVsTruePhi0FakeCand_"+(fitName)).c_str(), "; TP #phi_{0} (Fake Cand); Fitted #phi_{0}", 70, -3.5, 3.5, 70, -3.5, 3.5 );
    hisFitVsTrueD0FakeCand_[fitName]     = inputDir.make<TH2F>(("FitVsTrueD0FakeCand_"+(fitName)).c_str(), "; TP d_{0} (Fake Cand); Fitted d_{0}", 100, -1., 1., 100, -1., 1. );
    hisFitVsTrueZ0FakeCand_[fitName]     = inputDir.make<TH2F>(("FitVsTrueZ0FakeCand_"+(fitName)).c_str(), "; TP z_{0} (Fake Cand); Fitted z_{0}", 100, -25., 25., 100, -25., 25. );
    hisFitVsTrueEtaFakeCand_[fitName]    = inputDir.make<TH2F>(("FitVsTrueEtaFakeCand_"+(fitName)).c_str(), "; TP #eta (Fake Cand); Fitted #eta", 70, -3.5, 3.5, 70, -3.5, 3.5 );

    hisPtResVsTrueEta_[fitName]      = inputDir.make<TProfile>(("PtResVsTrueEta_"+(fitName)).c_str(), "q/p_{T} resolution; |#eta|; q/p_{T} resolution", 25, 0.0, 2.5);
    hisPhi0ResVsTrueEta_[fitName]    = inputDir.make<TProfile>(("PhiResVsTrueEta_"+(fitName)).c_str(), "#phi_{0} resolution; |#eta|; #phi_{0} resolution", 25, 0.0, 2.5);
    hisEtaResVsTrueEta_[fitName]     = inputDir.make<TProfile>(("EtaResVsTrueEta_"+(fitName)).c_str(), "#eta resolution; |#eta|; #eta resolution", 25, 0.0, 2.5);
    hisZ0ResVsTrueEta_[fitName]      = inputDir.make<TProfile>(("Z0ResVsTrueEta_"+(fitName)).c_str(), "z_{0} resolution; |#eta|; z_{0} resolution", 25, 0.0, 2.5);
    hisD0ResVsTrueEta_[fitName]      = inputDir.make<TProfile>(("D0ResVsTrueEta_"+(fitName)).c_str(), "d_{0} resolution; |#eta|; d_{0} resolution", 25, 0.0, 2.5);

    hisPtResVsTruePt_[fitName]      = inputDir.make<TProfile>(("PtResVsTruePt_"+(fitName)).c_str(), "q/p_{T} resolution; q/p_{T}; q/p_{T} resolution", 20, 0.0, 100.0);
    hisPhi0ResVsTruePt_[fitName]    = inputDir.make<TProfile>(("PhiResVsTruePt_"+(fitName)).c_str(), "#phi_{0} resolution; p_{T}; #phi_{0} resolution", 20, 0.0, 100.0);
    hisEtaResVsTruePt_[fitName]     = inputDir.make<TProfile>(("EtaResVsTruePt_"+(fitName)).c_str(), "#eta resolution; p_{T}; #eta resolution", 20, 0.0, 100.0);
    hisZ0ResVsTruePt_[fitName]      = inputDir.make<TProfile>(("Z0ResVsTruePt_"+(fitName)).c_str(), "z_{0} resolution; p_{T}; z_{0} resolution", 20, 0.0, 100.0);
    hisD0ResVsTruePt_[fitName]      = inputDir.make<TProfile>(("D0ResVsTruePt_"+(fitName)).c_str(), "d_{0} resolution; p_{T}; d_{0} resolution", 20, 0.0, 100.0);
    ...
    hisNumStubsKilledByFit_[fitName] = inputDir.make<TH2F>(("NumStubsKilledByFit_"+(fitName)).c_str(), "; All stubs killed by fit; Good stubs killed by fit", 10,-0.5,9.5,10,-0.5,9.5);
...
void Histos::fillTrackFitting( const InputData& inputData, const std::vector<std::pair<std::string,L1fittedTrack>>& mFittedTracks, float chi2dofCutPlots ) {  
  ...
  for ( const std::pair<std::string,L1fittedTrack>& fittedTrackPair : mFittedTracks ){
    ...
    nFittedTracks[j] += 1;
    if( fitTrk.chi2dof() <= chi2dofCutPlots ) nStubsOnTrack[j] += fitTrk.getNumStubs();
    ...
    if ( fitTrk.accepted() ) {
	  ...
	  hisFitVsTrueQinvPtGenCand_[j]->Fill( htTrk.qOverPt(), fitTrk.qOverPt() );
	  hisFitVsTruePhi0GenCand_[j]->Fill( htTrk.phi0(), fitTrk.phi0() );
	  hisFitVsTrueD0GenCand_[j]->Fill( htTrk.d0(), fitTrk.d0() );
	  hisFitVsTrueZ0GenCand_[j]->Fill( htTrk.z0(), fitTrk.z0() );
	  hisFitVsTrueEtaGenCand_[j]->Fill( htTrk.eta(), fitTrk.eta() );
	...
	if (tp == nullptr){
	  ...
	  hisFitVsTrueQinvPtFakeCand_[j]->Fill( htTrk.qOverPt(), fitTrk.qOverPt() );
	  hisFitVsTruePhi0FakeCand_[j]->Fill( htTrk.phi0(), fitTrk.phi0() );
	  hisFitVsTrueD0FakeCand_[j]->Fill( htTrk.d0(), fitTrk.d0() );
	  hisFitVsTrueZ0FakeCand_[j]->Fill( htTrk.z0(), fitTrk.z0() );
	  hisFitVsTrueEtaFakeCand_[j]->Fill( htTrk.eta(), fitTrk.eta() );
	}
    }
    ...
    hisNumStubsKilledByFit_[j]->Fill( fitTrk.getNumKilledStubs(), fitTrk.getNumKilledMatchedStubs() );  
    ...
    /*unsigned int nLayers = Utility::countLayers( settings_, fitTrk.getStubs() ); // Count tracker layers with stubs
      bool minLayers = nLayers >= settings_->minStubLayers(); // Set minimum number of layers
      if (fitTrk.getL1track3D().pt() > settings_->minPtToReduceLayers()) minLayers = nLayers >= settings_->minStubLayers()-1;
      profTrksKilledByFit_[j]->Fill(ibin, !minLayers);*/
	...
	hisTrueVsSeedQinvPtResGoodChi2_[j]->Fill( tp->qOverPt() - htTrk.qOverPt());
	hisTrueVsSeedPhi0ResGoodChi2_[j]->Fill( reco::deltaPhi(tp->phi0(), htTrk.phi0()) );
	hisTrueVsSeedD0ResGoodChi2_[j]->Fill( tp->d0() - htTrk.d0() );
	hisTrueVsSeedZ0ResGoodChi2_[j]->Fill( tp->z0() - htTrk.z0() );
	hisTrueVsSeedEtaResGoodChi2_[j]->Fill( tp->eta() - htTrk.eta() );  
	...
	if (fitTrk.getPurity() > 0.99) {    
	  ...
	  */
      ...
      // Plot helix parameter resolution against eta.

      hisPtResVsTrueEta_[j]->Fill( std::abs(tp->eta()), std::abs( fitTrk.qOverPt() - tp->qOverPt() ) );
      ...
      hisPtResVsTruePt_[j]->Fill( std::abs(1/tp->qOverPt()), std::abs( fitTrk.qOverPt() - tp->qOverPt() ) );
      hisPhi0ResVsTruePt_[j]->Fill( std::abs(1/tp->qOverPt()), std::abs(reco::deltaPhi(fitTrk.phi0(), tp->phi0()) ) );
      hisEtaResVsTruePt_[j]->Fill( std::abs(1/tp->qOverPt()), std::abs( fitTrk.eta() - tp->eta() ) );
      hisZ0ResVsTruePt_[j]->Fill( std::abs(1/tp->qOverPt()), std::abs( fitTrk.z0() - tp->z0() ) );
      hisD0ResVsTruePt_[j]->Fill( std::abs(1/tp->qOverPt()), std::abs( fitTrk.d0() - tp->d0() ) );
