...
L1track3D L1track3D::mergeTracks(const L1track3D B) const {
  ...
  unsigned int aP=0, bP=0, aS=aStubs[aP]->index(), bS=bStubs[bP]->index();
  ...
  // & software.
  return L1track3D(settings_, mStubs, this->getCellLocationRphi(), this->getHelixRphi(), 
	                              this->getCellLocationRz()  , this->getHelixRz());
}
