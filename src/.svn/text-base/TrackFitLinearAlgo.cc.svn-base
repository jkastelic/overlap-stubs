...
TrackFitLinearAlgo::TrackFitLinearAlgo( const Settings* settings, const uint nPar ) : TrackFitGeneric(settings), settings_(settings), print(settings_->debug() ) {
  ...
  minStubLayers_ = settings->minStubLayers(); // Min. number of layers that must have stubs for track to be declared found.
  minPtToReduceLayers_ = settings->minPtToReduceLayers(); // Reduce MinStubLayers by 1 for tracks with pt above this cut.
